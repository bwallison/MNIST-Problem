...
def main():
    ...
    #define hyperparameters for cross-validation gridsearch
    param_grid = {'C': [0.001, 0.01, 0.1, 1, 10, 100, 1000] }
    
    #create classifiers from specified hyperparameters
    classifier = GridSearchCV(LogisticRegression(solver = 'lbfgs'), param_grid)

    #fit to training data
    best_model = classifier.fit(train_data, train_labels)

    #print the best penalty and c measure
    print('Best Penalty:', best_model.best_estimator_.get_params()['penalty'])
    print('Best C:', best_model.best_estimator_.get_params()['C'])
    
    #retrain with best hyper parameters
    classifier = LogisticRegression(solver = 'lbfgs', C=best_model.best_estimator_.get_params()['C'], penalty=best_model.best_estimator_.get_params()['penalty'])
    ...
    #predict the value of the test data with logistic model
